...
#endif
byte modbus_data_first[]={Modbus_Addr,0x03,0x10,0x00,0x00,0x13,0x00,0xc7};
...
UINT16 GenericApp_ProcessEvent( byte task_id, UINT16 events )
{
    ...
    {
      //HalLedSet(HAL_LED_1,HAL_LED_MODE_OFF);//LED1闪烁一次表示发送 ModBus数据
      ...
      {
        case First_Seq:
          HalUARTWrite(0, modbus_data_first, 8);//定时发送发送数据到汇流箱
          modbus_state=Second_Seq;
          break;
        case Second_Seq:
          HalUARTWrite(0, modbus_data_second, 8);//定时发送发送数据到汇流箱
          modbus_state=Third_Seq;
          break;
        case Third_Seq:
          HalUARTWrite(0, modbus_data_third, 8);//定时发送发送数据到汇流箱
          modbus_state=First_Seq;
          break;
        default:
          break;
    ...
    if (SerialApp_TxLen)
    {
      if(RxBuf[0]==Modbus_Addr&&RxBuf[1]==0x03)
      {
        ...
        if(crc_high==RxBuf[len+3]&&crc_low==RxBuf[len+4])//确保从modbus_485接收到的数据没有错.
        {
          ...
          {
            case First_Seq:
              modus_first_deal(RxBuf+3,len);
              break;
            case Second_Seq:
              modus_second_deal(RxBuf+3,len);
              break;
            case Third_Seq:
              modus_third_deal(RxBuf+3,len);
              break;
          }
          //crc=CRC16_Check(RxBuf,len+3);
          //RxBuf[len+3]=(crc>>8)&0xff;
          //RxBuf[len+4]=crc&0xff;
          GenericApp_SendTheMessage(RxBuf,len+5);//发送数据到协调器
        }
      }
...
void modus_first_deal(byte *RxBuf,uint8 len)
{
  ...
  for(i=0;i<len;i++)
  {
    ...
    {
      flag|=(1<<(15-i/2));
      RadioBuf[7+i-1]=RxBuf[i-1];
      RadioBuf[7+i]=RxBuf[i];
    }
  }
  RadioBuf[2]=flag>>8&0xff;
  RadioBuf[3]=flag&0xff;
}
