...
byte *eui64;
static int group=sizeof(d_message)/sizeof(D_Message_t);
static int group_index=0;
static int time_interal=0;
//#ifdef LOW_THROUGHPUT
int radio_send_flag=1;
...
UINT16 GenericApp_ProcessEvent( byte task_id, UINT16 events )
{
    ...
    if ( events & GENERICAPP_SEND_MSG_EVT )
    {
      if(group_index>=group){
        radio_send_flag=0;
        time_interal++;
        if(time_interal>=INTERVAL_SAMPLE_ACTUAL/GENERICAPP_SEND_MSG_TIMEOUT*INTERVAL_SAMPLE){
          radio_send_flag=1;
          time_interal=0;
        }
      }   
      if(radio_send_flag==1){      
