...
byte *eui64;
byte Radio_GPRS_Buf[2][64];
uint8 uart_rx_index=0;
...
void GenericApp_Init( byte task_id )
{
  /**/
    ...
    Radio_GPRS_Buf[0][0]=SERIAL_APP_RADIO_HEAD1;
    Radio_GPRS_Buf[0][1]=SERIAL_APP_RADIO_HEAD2;
    Radio_GPRS_Buf[1][0]=SERIAL_APP_RADIO_HEAD1;
    Radio_GPRS_Buf[1][1]=SERIAL_APP_RADIO_HEAD2;
...
void GenericApp_MessageMSGCB(afIncomingMSGPacket_t *pkt)
{
    ...
    case GENERICAPP_CLUSTERID:
        for(i=0;i<pkt->cmd.DataLength;i++)
        {
          ...
          {
            //Radio_GPRS_Buf[uart_rx_index][uart_len++]=SERIAL_APP_RADIO_HEAD1;//head
            //Radio_GPRS_Buf[uart_rx_index][uart_len++]=SERIAL_APP_RADIO_HEAD2;//head
            playload_len=uart_len++;
            osal_memcpy(Radio_GPRS_Buf[uart_rx_index]+uart_len,eui64,8);//coordinator address
            uart_len+=8;
            osal_memcpy(Radio_GPRS_Buf[uart_rx_index]+uart_len, pkt->cmd.Data+2, 10);//endvice address without head
            uart_len+=10;
            osal_memcpy(Radio_GPRS_Buf[uart_rx_index]+uart_len, pkt->cmd.Data+14, pkt->cmd.DataLength-14); 
            uart_len+=pkt->cmd.DataLength-14;
            Radio_GPRS_Buf[uart_rx_index][playload_len]=uart_len-playload_len-1;//payload length
            crc=CRC16_Check(Radio_GPRS_Buf[uart_rx_index],uart_len);
            Radio_GPRS_Buf[uart_rx_index][uart_len++]=crc>>8&0xff;
            Radio_GPRS_Buf[uart_rx_index][uart_len++]=crc&0xff;
#ifdef HIGH_THROUGHPUT
            uart_rx_index++;
            if(uart_rx_index>=2)
            {
              uart_rx_index=0;
              HalUARTWrite(0, Radio_GPRS_Buf[0], Radio_GPRS_Buf[0][2]+5);
              HalUARTWrite(0, Radio_GPRS_Buf[1], Radio_GPRS_Buf[1][2]+5);
            }  
#elif LOW_THROUGHPUT      
            HalUARTWrite(0, Radio_GPRS_Buf[uart_rx_index], Radio_GPRS_Buf[uart_rx_index][2]+5);
            uart_rx_index%=2;
#endif     
            break;
          }
